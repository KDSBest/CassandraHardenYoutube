...
(
    ...
    while ! cqlsh -u cassandra -p cassandra -e 'describe cluster' > /dev/null 2>&1; do echo "Wait for cassandra cluster"; sleep 6; done
    echo "$0: Cassandra cluster ready: change password"
	envsubst < /etc/cassandra/changepw.template.cql > /etc/cassandra/changepw.cql
	cqlsh -u cassandra -p cassandra -f /etc/cassandra/changepw.cql
) &
